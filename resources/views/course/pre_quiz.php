...
if (isset($_GET['course_id']) && $_GET['course_id'] > 0) {
    ...
    $sql = "SELECT id_members_course_status, course_id, pre_evalution FROM members_course_status WHERE member_id=" . $member['id_members'] . " AND course_id=$course_id LIMIT 1";
    ...
    } else if ($course_status['pre_evalution']) {
        $sql = "SELECT id_lessons FROM lessons WHERE courses_id=$course_id ORDER BY lesson_no ASC LIMIT 1";
        $result = $conn->query($sql);
        $lesson_id = $result->fetch_assoc();
?>
    <div class="container py-4">
        ...
        <div class="card w-100 w-lg-80 mx-auto p-2 p-lg-5 m-5 rounded" style="border-radius: 15px;" id="result-div">
            <div class="card-body text-center">
                ...
                <br>
                <a href="lesson.php?lesson_id=<?= $lesson_id['id_lessons'] ?>" class="p-3 btn btn-danger rounded-pill fw-bold"> এই কোর্সটির প্রথম পাঠ সম্পন্ন করতে এখানে ক্লিক করুন <i class="fas fa-arrow-right"></i></a>
            </div>
        </div>
    </div>
    ...
    <?php
        ...
        $sql = "SELECT id_questions, course_id, qus_cat, ques_type, question FROM questions WHERE qus_cat LIKE '%pre_evaluation%' AND ques_type='radio'";
        $result = $conn->query($sql);
        $radio_questions = $result->fetch_all(MYSQLI_ASSOC);

        $sql = "SELECT id_questions, course_id, qus_cat, ques_type, question FROM questions WHERE qus_cat LIKE '%pre_evaluation%' AND ques_type='checkbox'";
        $result = $conn->query($sql);
    ...
    ?>
        ...
        <div class="container-xxl py-4 mb-5">
            <div class="container">
                ...
                <?php if ($error = get_error('pre_quiz_error')) { ?>
...
if (isset($_POST['submit_answer'])) {
    ...
    $sql = "INSERT INTO survey(member_id, status_table_id, evaluation_type, group_a, group_b, created_at) VALUES ($member_id, $status_table_id, 'pre', '$group_a', '$group_b', now())";
    $result = $conn->query($sql);

    if ($result) {
        $sql = "UPDATE members_course_status SET pre_evalution=1,  course_status='continue', started_at=now() WHERE id_members_course_status=$status_table_id";
        $result = $conn->query($sql);
        header("Location: pre_quiz.php?course_id=" . $course_status['course_id']);
    } else {
        set_error('pre_quiz_error', 'Something Went Wrong! Please try again later.');
    }
}
